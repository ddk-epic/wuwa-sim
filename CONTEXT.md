...
The two checks split by layer. **Existence/ordering is validation-time** ([[validate-timeline]], which stays frame-agnostic): chain mode walks back to the immediate predecessor; window mode scans the same character's whole prior timeline for a matching cast, hard-erroring if none — guaranteeing the sim finds an anchor. **Timing is simulation-time**: the sim records each prerequisite's cast frame (most-recent-wins) and pads the follow-up's start by `max(0, anchorFrame + minDelay − cursor)`, a [[Padding Delay]] component `max`-combined with [[Swap-back Cooldown]]'s `swapBack`. Too-soon is **padded, never errored** — a forced wait, like Swap-back Cooldown. The anchor is the cast frame, recorded even on a swap-cancel (cast-time effects fire per ADR-0018), so an [[In-trailing]] swap-back needs no dedicated machinery: by the time `minDelay` elapses the swap stage's [[Trailing Window]] has closed anyway. The window _end_ (an upper bound past which the follow-up lapses) is deferred — no `maxDelay` field exists yet.
