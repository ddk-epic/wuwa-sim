...
**Padding Delay**:
Frames added to an entry's `advance` (or its startup) by the engine, on top of any variant-authored cost. Six sources today: (1) Reaction Delay (applied automatically to variant-tagged entries), (2) [[Variant Floor]] (raises `advance` to the floor when `actionTime + Reaction Delay` is below it), (3) Trailing Window collisions where a non-cancel-capable same-character re-entry forces the **immediately preceding** entry's `advance` to extend so the re-entry can fit, (4) [[Fall Frames]] when a stage's required entry footing is `ground` while current team footing is `air`, (5) [[Swap-back Cooldown]] when a character is swapped back in before their 60-frame off-field clock elapses, and (6) the windowed [[Prior-Stage Gate]] when a follow-up entry's start is earlier than `prerequisiteCastFrame + minDelay`. Surfaced on the Timeline row and Simulation Log entry split by kind: the **start floors** `swapBack` and `priorGate` (the entry can't begin until the swap-back CD and gate elapse — the character waits, idle) render as an up-front **wait** badge in a dedicated column before the action, summed together; the **action costs** `react`/`floor`/`pad`/`fall` form the merged `+0.Xs` delay suffix beside the skill, with a tooltip splitting them. The wait is also kept **out of the preceding row's Duration cell** (which is the gap to the next action), so it appears once — as the next entry's wait — rather than doubled into the prior entry's length; the rotation total and DPS clock still count it. The `react` and `floor` components are **mutually exclusive** (only whichever wins the `max` is shown); `pad` and `fall` are additive on top, while the Prior-Stage-Gate floor `max`-combines with `swapBack` (both are absolute-frame floors on the same entry's start, so the wait badge sums them without double-counting). Attribution placement differs: `react`, `floor`, `fall`, `swapBack`, and the Prior-Stage-Gate component attach to the current entry; `pad` attaches to the immediately preceding entry.
...
The two checks split by layer. **Existence/ordering is validation-time** ([[validate-timeline]], which stays frame-agnostic): chain mode walks back to the immediate predecessor; window mode scans the same character's whole prior timeline for a matching cast, hard-erroring if none — guaranteeing the sim finds an anchor. **Timing is simulation-time**: the sim records each prerequisite's cast frame (most-recent-wins) and pads the follow-up's start by `max(0, anchorFrame + minDelay − cursor)`, a [[Padding Delay]] component `max`-combined with [[Swap-back Cooldown]]'s `swapBack`. Too-soon is **padded, never errored** — a forced wait, like Swap-back Cooldown. The anchor is the cast frame, recorded even on a swap-cancel (cast-time effects fire per ADR-0018). An [[In-trailing]] swap-back still has residue in flight when the gated follow-up fires — `minDelay` (103) lands _inside_ the [[Trailing Window]] (108), with the prerequisite's last hit at the same frame as `minDelay`. The follow-up must therefore resolve its cancel against its **`effectiveStart`**, not the pre-pad cursor, or it wrongly drops residue that lands during the gate wait (ADR-0037 — the case ADR-0036 mistakenly assumed needed no machinery). The window _end_ (an upper bound past which the follow-up lapses) is deferred — no `maxDelay` field exists yet.
