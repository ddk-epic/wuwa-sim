...
**Footing**:
Team-level vertical-position state, one of `ground | air`. A Stage optionally declares `footing: "ground" | "air" | "launch" | "land"` — the value encodes both required entry footing and exit footing (`launch` = ground→air, `land` = air→ground). Omission means the stage is footing-transparent: no validity check, no state change (Dodge is the canonical case; Jump is `launch`). The engine tracks current team footing and uses it to validate the next stage: `current=ground` against a stage requiring `air` is a hard error ("launch/jump required"); `current=air` against a stage requiring `ground` is soft and triggers [[Fall Frames]] as a new [[Padding Delay]] component. The hard cases `air→launch` and `ground→land` also error. Validation is timeline-authoring-only (parallel to `requiresStageId` and `comboAllows`); the engine trusts valid timelines. Different-footing variants of the same conceptual move are authored as **separate fundamental Stages**, not as a single polymorphic stage.

**FootingTracker**:
The engine collaborator that owns Footing state. Sibling to [[On-Field Tracker]] — same shape (one nullable value, mutation-versioned) but entirely orthogonal: swap inference and footing inference do not read each other. Carries two pieces of state: (1) **team footing** — updated by every resolved footing-bearing Stage to that stage's exit footing, and (2) a per-character **trailing-window footing snapshot** dict — populated on swap-out while the swap-variant's [[Trailing Window]] is active, cleared when that window expires or the character re-enters. On swap-in, the incoming character's first stage validates against team footing — _unless_ the incoming character has a live snapshot, in which case the snapshot overrides team footing only for that re-entry. After the re-entered stage resolves, team footing follows its exit value as normal.
...
A trailing window also preserves the swapped-out character's exit [[Footing]] as a snapshot on the [[FootingTracker]] for the duration of the window. If the same character re-enters while their window is still open, the snapshot overrides current team footing for that re-entry's validity check (the character is "still airborne" even if the team's footing has since transitioned via other characters' stages). Snapshot is cleared on re-entry or window expiry.
...
**Padding Delay**:
Frames added to an entry's `advance` (or its startup) by the engine, on top of any variant-authored cost. Four sources today: (1) Reaction Delay (applied automatically to variant-tagged entries), (2) [[Variant Floor]] (raises `advance` to the floor when `actionTime + Reaction Delay` is below it), (3) Trailing Window collisions where a non-cancel-capable same-character re-entry forces the **immediately preceding** entry's `advance` to extend so the re-entry can fit, and (4) [[Fall Frames]] when a stage's required entry footing is `ground` while current team footing is `air`. Surfaced on the Timeline row and Simulation Log entry as a merged `+0.Xs` suffix in seconds; a tooltip splits the components for forensics. The `react` and `floor` components are **mutually exclusive** (only whichever wins the `max` is shown); `pad` and `fall` are both additive on top. Attribution placement differs: `react`, `floor`, and `fall` attach to the current entry; `pad` attaches to the immediately preceding entry.

**Movement Stage**:
A first-class Stage modeling a universal player movement action (`Dodge`, `Jump`) that exists on every character. Authored centrally in `src/data/movement.ts` and injected into every character's `skills` at enrichment time — never written in raw character JSON. Carries a hardcoded `actionTime` (Dodge 21 frames, Jump 18 frames at 60fps), empty `damage`, and `type: "Movement"`. Reaches the timeline as a normal Timeline Entry and emits an Action Event for log visibility, but **bypasses the Phase Pipeline** — no `skillCast` dispatch, no resource deltas, no cooldown roll, no `hitLanded` (no damage entries). The buff engine still `tickToFrame`s past the stage so duration-based expirations advance. Reaction Delay does not apply. [[Footing]] assignments: `Jump = "launch"` (ground→air, hard error in `air`); `Dodge` omits `footing` (transparent — valid in both footings, preserves current). See ADR-0015.
