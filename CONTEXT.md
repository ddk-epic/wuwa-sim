...
**FootingTracker**:
The engine collaborator that owns Footing state. Sibling to [[On-Field Tracker]] — same shape (mutation-versioned) but entirely orthogonal: swap inference and footing inference do not read each other. Carries two pieces of state: (1) **team footing** — the on-field character's current vertical state, updated by launch/land commit events fired by the [[Trailing Window]] when the owning character is on-field at the commit frame, and (2) a per-character **trailing-window footing snapshot** dict — populated when a launch/land commit fires while the owning character is _off-field_ (i.e. mid-trailing-window after a swap-cancel that crossed the point-of-no-return), cleared when that window expires or the character re-enters. On swap-in, the engine consumes the incoming character's snapshot if any and **promotes it to team footing** (the on-field invariant: team footing reflects whoever is currently on-field) before the entry's footing validity check; if no snapshot exists, the incoming character simply inherits current team footing. After re-entry, subsequent launch/land commits update team footing in place via the on-field path. Public surface: `current()`, `setTeam(footing)`, `snapshotFor(charId, footing)`, `consumeSnapshot(charId)`, `clearSnapshot(charId)`. See ADR-0022 and its Launch/Land amendment.
