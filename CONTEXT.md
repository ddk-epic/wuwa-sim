...
**FootingTracker**:
Low-level state holder for Footing. Sibling to [[On-Field Tracker]] — same shape (mutation-versioned) but entirely orthogonal: swap inference and footing inference do not read each other. Carries a **single** piece of state: **team footing** — the on-field character's current vertical state (`ground | air`). No per-character state — footing is read only for the on-field character, so an off-field character's vertical divergence is parked in the [[Trailing Window]]'s `pendingFooting` (already per-character) until that character re-enters. Private collaborator of `FootingModule`; not accessed directly from `simulation.ts`. See ADR-0022 and its snapshot-removed amendment.

**FootingModule**:
Owns `FootingTracker` (a private collaborator) and applies Footing changes. Exposed on `BuffEngine` as `engine.footing`. Operations: `applyStageFooting(characterId, footing, stageDuration)` — commits a launch/land event whose frame falls within the stage duration; `commit(exitFooting)` — sets team footing to a surviving deferred launch/land, called by the [[Trailing Window]] at the owner's re-entry (owner is on-field at that frame, so "their footing" _is_ team footing); `current(): "ground" | "air"` — reads team footing (the swap-in path is now a plain `current()` read, no snapshot to consume). Dependency is one-way: the Trailing Window calls into `FootingModule`; `FootingModule` knows nothing of swaps or tails. See ADR-0022 and its snapshot-removed amendment.
...
A trailing window also carries the swap stage's [[Footing]] commit (if the stage declares `{ launch: number }` or `{ land: number }`) as a per-entry **pending footing event** alongside its trailing hits. The event shape lives on the trailing-window state as `TrailingEntry { hits, pendingFooting? }` — at most one per in-flight stage (one launch/land frame per stage by construction). `pendingFooting` is the **sole** per-character carrier of an in-flight launch/land while its owner is off-field (there is no separate footing snapshot — see [[FootingTracker]]). The commit is applied at the owner's **re-entry**: if it survives, the trailing window calls `engine.footing.commit(exitFooting)` (the owner is on-field at that frame). Cancel-capable drop semantics apply to the footing event identically to hits: if a same-character cancel-capable re-entry drops events past `newEntry.startFrame` and `pendingFooting.atFrame ≥ newEntry.startFrame`, the commit drops (no team flip — the launch never happened). The non-cancel-capable pad target frame is `max(lastHitFrame, pendingFooting?.atFrame ?? -∞)`.
