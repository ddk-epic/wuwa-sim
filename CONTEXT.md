...
**Footing**:
Vertical-position state, one of `ground | air`, tracked **per character** — every party member carries its own footing (see [[Deployment State]]), not a single shared value. **Team footing** — the value the engine reads for entry validation, [[Fall Frames]], and footing-conditional buffs — is a _derived_ view: it is the **On-field** character's footing. A character that goes airborne mid-swap stays `air` while [[In-trailing]] and reverts to `ground` on becoming [[In-reserve]]; the next character to swap in is read on **its own** footing, which is why a swap-back during the trailing window can enter `air` while the character who held the field stayed `ground`. A Stage optionally declares `footing: "ground" | "air" | { launch: number } | { land: number }` — sustained values are terse strings; transitions carry the in-stage **commit frame** (the action-frame at which the vertical-position change actually occurs in the animation, e.g. the frame the body leaves the ground for `launch`). `{ launch }` requires entry `ground`, exits `air`; `{ land }` requires entry `air`, exits `ground`. Omission means the stage is footing-transparent: no validity check, no state change (Dodge is the canonical case; Jump is `{ launch: ... }`). The engine validates the next stage's required entry footing against current team footing: `current=ground` against a stage requiring `air` is a hard error ("launch/jump required"); `current=air` against a stage requiring `ground` is soft and triggers [[Fall Frames]] as a new [[Padding Delay]] component. The hard cases `air→launch` and `ground→land` also error. Validation is timeline-authoring-only; the engine trusts valid timelines. Different-footing variants of the same conceptual move are authored as **separate fundamental Stages**, not as a single polymorphic stage. The commit frame on `{ launch }` / `{ land }` is a point-of-no-return: a swap-cancel whose `actionTime` lands before the commit frame does **not** complete the transition (the launch never happens). The commit is an **instant** event (single-frame flip — footing is binary, there is no transition easing) on the simulator's frame-ordered stream: it fires at its commit frame and sets **its owner's** footing (`air` on `launch`, `ground` on `land`), whether or not that owner is currently On-field. A same-character cancel-capable re-entry before the commit frame **drops** it (the launch never happened); a non-cancel re-entry pads past it so it fires. See ADR-0022 (per-character footing amendment) and ADR-0028.

**FootingTracker**:
Low-level state holder for Footing. Sibling to [[On-Field Tracker]] — same shape (mutation-versioned). Carries a **per-character** map of vertical state (`ground | air`, default `ground`), one entry per party member — an [[In-trailing]] character's footing can diverge from the On-field character's. **Team footing** is not stored; it is derived by reading the On-field character's entry (the tracker collaborates with [[On-Field Tracker]] to know who that is), so a swap-in automatically reads the incoming character on its own footing. Private collaborator of `FootingModule`; not accessed directly from `simulation.ts`. See ADR-0022 (per-character footing amendment).

**FootingModule**:
Owns `FootingTracker` (a private collaborator) and applies Footing changes. Exposed on `BuffEngine` as `engine.footing`. Operations: `applyStageFooting(characterId, footing, stageDuration)` — commits a launch/land whose commit frame falls within the stage duration directly to that character's footing; `commitFor(characterId, exitFooting)` — sets one character's footing, the operation a footing stream event (commit or window-end reset) invokes; `current(): "ground" | "air"` — returns **team footing** by reading the On-field character's per-character entry (no snapshot to consume; a swap-in is a plain `current()` read that now lands on the incoming character's own footing). See ADR-0022 (per-character footing amendment).
