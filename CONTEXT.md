...
**FootingModule**:
Owns `FootingTracker` (a private collaborator) and applies Footing changes. Exposed on `BuffEngine` as `engine.footing`. Operations: `team(): "ground" | "air"` — current team footing (the On-field character's); `applyStageFooting(footing, stageDuration)` — an on-field launch/land whose commit frame falls within the stage's own duration flips **team** footing immediately (the character never leaves the field, so it is not a stream event); `commitFor(characterId, exitFooting)` — sets one character's **carried** footing, the operation a footing stream event (an [[In-trailing]] commit or the window-end reset) invokes; `resolveEntry(characterId): "ground" | "air"` — the footing a character takes the field on as it begins an entry: its carried override if any (consumed and promoted to team), otherwise inherited team footing. See ADR-0022 (per-character footing amendment) and `references/footing.md`.
...
A per-team boolean [[Settings]] flag (default `false`) that seeds every character's energy bar at max for the run, feeding the corresponding positional `runSimulation` parameter. Unlike the four frame knobs it is not clamped — it is the one non-frame member of the otherwise number-shaped Settings bag.
