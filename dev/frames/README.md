...
The clip set persists to **localStorage, keyed per character**, so a reload doesn't wipe a measurement campaign (the accumulation of clips is the whole point of differencing). The write is **debounced off the edit hot path** (`useDebouncedSave`): every edit updates in-memory state (`clips` + `clipsRef`) synchronously so reads stay correct, but the `JSON.stringify` + `setItem` coalesces on a trailing timer, flushed before a character switch, on unmount, and on `beforeunload` so no edit is lost. The debounced payload carries its own character, so a write landing after a switch still targets the right key.
