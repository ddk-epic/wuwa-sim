...
```
D = MV × scaling × (1 + bonusMultiplier) × (1 + dmgBonus) × (1 + amp) × (1 + vul) × crit × def × res
                    └─ separate ───┘        └─ additive ──┘   └ additive ┘  └─ separate ┘
                       multiplier             DMG-Bonus        Amplify       vulnerability
```

`dmgBonus` and `amp` each sum their contributions additively within their
...
| "DMG is **Amplified by** N%" / "**Amplified**"                             | **Amplify** (additive)                      | `allAmp`, `elementAmp[…]`, `skillTypeAmp[…]`             |
...
- **加深** ("deepen / intensify") → **Amplify** bucket (the `amp` additive space).

When a description is ambiguous about whether an effect is a DMG Bonus or an
Amplify, double-check against the Chinese wording (提升 vs. 加深) before deciding.
...
can actually denote a **Bonus Multiplier** (or an Amplify). Confirm the Chinese
