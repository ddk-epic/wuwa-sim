...
## 2026-07-05 — Skill cooldown exempts same-skill follow-ups

Refines ADR-0045's "skill-keyed cooldown pools across every stage." A stage that continues a prior stage of the **same skill** (its `requiresPriorStage` names another stage under the same `skillKey`) is one activation, not a re-press, so it neither arms nor is gated by the skill timer — `findStageByEntry` leaves its `skillCooldown` unset, and `cooldownTimers` (unchanged) simply finds no skill-keyed timer for it (stage-keyed timers and genuine sibling stages are unchanged). Fixes Encore's Energetic Welcome spuriously flagging `skillOnCooldown` behind Flaming Woolies' 10s cooldown, and covers the chained Cosmos: Frolicking follow-ups too. Independent of the follow-up's gate mode (window vs. chain): the signal is `requiresPriorStage` same-skill. Latent case flagged: Cosmos: Frolicking Stage 1 and Cosmos Rampage carry no `requiresPriorStage`, so they'd still read the liberation's pooled timer if placed inside it — the deeper "initiating cast owns the cooldown" rework stays deferred.

Pages touched: CONTEXT.md (Skill Cooldown).
...
Pages touched: ADR-0040 (new), ADR-0027 (lead corrected), CONTEXT.md (Settings + Start With Full Energy terms; Reaction Delay/Swap Frames/Variant Floor/Fall Frames/Session/Active Team updated).

## 2026-06-11 — Footing and resource warnings become engine-emitted Diagnostics; the validator's footing walk is deleted

An architecture review flagged footing as "one rulebook, two interpreters": `validateTimeline` carried a static mirror of the engine's footing semantics (variant-aware exit inference, two near-identical walk passes), and the audit found it had drifted — it compared commit frames against raw `actionTime`s while the sim used the padded advance, because the validator never receives Reaction Delay. The first instinct (extract a shared footing rulebook both sides consume) was rejected as doubling down on a layer violation: footing legality is frame-dependent, and ADR-0036 already ruled that the validator stays frame-agnostic. Instead the second interpreter died. New `Diagnostic` shape (`footingViolation` / `insufficientEnergy` / `insufficientConcerto`) on `ActionEvent.diagnostics`: the sim checks entry footing where it already holds the true state (~5 lines, Intro exempt), and the two `console.warn`s for below-cost Liberation/Outro casts now ride the same field (`onEvent` returns `diagnostics`). Everything executes unchanged — warn-but-proceed. The validator's footing machinery (~140 lines) is deleted, not moved; its fall warning is dropped outright (the row already shows `fall` in the delay split). `TimelineView` folds log Diagnostics into the row warning channel by `sourceEntryId`, and warning-only rows now qualify for the inline message (previously only error rows did, so warnings never displayed). Trade-off accepted: footing/resource feedback is as fresh as the last run (stale regime), while structural checks stay per-keystroke. Do not re-add a static footing predictor.

Pages touched: ADR-0022 (validity section folded to run-time Diagnostics; old static walk retrofitted into Considered Options), CONTEXT.md (new Diagnostic term; Footing, Resonance Cost).
