...
- **Advisory consequence findings state the consequence, not a violated precondition.** When the engine applied a legal automatic consequence and nothing was violated (`footingFall` — fall padding inserted before a grounded entry; `footingForced` — a `forcesFooting` mode overriding a conflicting stage entry), the message states the **consequence applied** rather than a requirement. The surface rules are unchanged: impersonal, sentence case, no trailing period, no em-dash, and omit anything the row already shows (the `+N` `DelayBadge` already carries the frame count, so the wording is frame-free). `footingForced` supersedes both other footing findings on the entry it fires: while a mode forces a footing, a conflicting stage cannot raise `footingFall` (the fall is suppressed to zero) or `footingViolation` (the entry runs on the forced footing). Its `footing` field names the forced direction so the wording can pair the air and ground cases ("Air-only mode forces an aerial entry" / "Ground-only mode forces a grounded entry").
