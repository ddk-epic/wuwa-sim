...
- **Gated (suspendable) duration** — a timed duration (`frames`/`seconds`) may carry `while: { buff: <id> }`: the timer only elapses while that gate buff is present on the same target. Applying with the gate absent starts the instance paused (`endTime` infinity, full length banked in `gatedRemaining`). Removing the gate buff banks `endTime − frame` and freezes `endTime` to infinity; re-applying it resumes with `endTime = frame + gatedRemaining`. Gates are indexed by gate-buff id (`gatedByGate`), so pause/resume runs off that buff's own apply and removal paths rather than scanning conditions. This is the one place `endTime` is **mutable and non-monotonic** after apply — `latestActiveEndFrame` and the trailing-expiry flush read it live and must never cache it.
...
- **buffExpired propagation** — a buff can trigger on another buff ending via `trigger: { event: "buffExpired", buff: <id> }`. When an instance is removed inside a dispatch context (the `removeBuffs` phase or source swap-out), `dispatchExpiries` synthesizes a `buffExpired` engine event per removed instance and re-dispatches it, so removals propagate identically regardless of what ended the buff. Each instance is dropped from `active` before its `buffExpired` fires, so a cascade of removals that re-ends the same buff finds nothing and terminates.
- **Timer-expiry propagation** — instances pruned by `endTime ≤ frame` are pruned inside `tickToFrame` (`InstanceStore`), outside any dispatch context. The store's `tickToFrame` returns those `expired` instances; `BuffEngine.tickToFrame` then runs `dispatchTimerExpiries` for each — both in the per-tick loop and on the final advance — so a buff ending by timer fans out through `buffExpired` exactly as an explicit removal does. Dispatched at positive depth, so any in-frame synthetic hits/sustains a timer-triggered buff produces resolve inline into the tick stream rather than deferring.
