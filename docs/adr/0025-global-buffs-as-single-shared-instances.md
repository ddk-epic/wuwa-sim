...
- The 6 Stellarealm/Butterfly defs fold into `shorekeeper.ts` (a global buff is triggered from its owner's `triggerableBySource` bucket, so it must be seeded on the owner's slot path). `src/data/global-buffs.ts`, the bootstrap loop (`buff-engine.ts:246-260`) + its import, and the `owner` field on `BuffDef` are all **deleted** — sequence gating is already handled by `resolveCharacterBuffs` (`loadout/resolve-character.ts`, formerly `buildCharacterBuffDefs`), and `owner` had no other reader.
