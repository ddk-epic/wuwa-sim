...
| --------- | ----------- | ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Character | `Character` | `EnrichedCharacter` | per-stage `actionTime`, `variants` (cancel/instantCancel/swap), `footing`, `animationFrames`, stage `key`/`newName`/`newSkillName` (display-prefix override, default = skill name), combo lineage (`requiresPriorStageId`); `buffs` go from name scaffolds to `BuffDef[]`; adds `forteCap`, `template` |
