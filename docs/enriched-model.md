...
| --------- | ----------- | ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Character | `Character` | `EnrichedCharacter` | per-stage `actionTime`, `variants` (cancel/instantCancel/swap), `footing`, `animationFrames`, stage `key`/`newName`/`newSkillName` (display-prefix override, default = skill name), combo lineage (`requiresPriorStage`, a `"skill/stage"` token or an any-of array that may include the reserved `"swap-in"` opener sentinel); `buffs` go from name scaffolds to `BuffDef[]`; adds `forteCap`, `template` |
