...
4. **The frame-ordered stream** (`simulation.ts`) — synthetic emits, swap-stage trailing hits, and footing commits are all members of one pending stream keyed by landing frame. Before every engine advance, `advanceTo(ctx, F)` drains the stream `≤ F` first — the engine clock is monotonic (`tickToFrame` only moves forward), so a member must resolve before the clock passes its frame or its snapshot would be taken at the overshot frame. Emissions are footing-blind and fire-and-forget; trailing hits and footing commits carry their ADR-0018/0022 drop/pad/reset semantics by marking stream members invalid + a cursor rule.
