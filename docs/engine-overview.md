...
2. **Timeline walk** (`src/lib/stage/stage.ts` and the simulator loop) — iterate Timeline Entries in order. For each entry, fetch its Stage, apply any Stage Variant, schedule its Damage Entries against the frame clock.
...
- **`stage/`** — stage resolution: `stage`, `focused-stage-catalog`, `trailing-window`.
