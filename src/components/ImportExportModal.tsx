    ...
    <Modal variant="centered" onClose={onClose} title="Import / Export">
      <div className="flex flex-col gap-6 pt-4">
        <section className="flex flex-col gap-2">
          ...
          <ImportSection onImport={onImport} />
        </section>
        <section className="flex flex-col gap-2">
          <h3 className="text-sm font-semibold text-gray-300">Export</h3>
          <ExportSection exportString={exportString} />
        </section>
      </div>
    </Modal>
  )
...
function ImportSection({ onImport }: { onImport: (value: string) => void }) {
  return (
...
function ExportSection({ exportString }: { exportString: string }) {
  return (
    <div className="flex flex-col gap-2">
      <textarea
