...
### 3a. Swap-in opener (`swap-in` sentinel)

**When to use**: a Basic Attack Stage 2 that also opens directly when its character is freshly swapped in (the near-universal tech where the first Normal Attack after a swap starts at Stage 2, not Stage 1).
...
| `skillTreeBonuses`         | Array of `SkillTreeStat` strings; compiled into permanent stat buffs at bootstrap (`compileSkillTreeNode`). The base-stat node (ATK/HP/DEF) also drives the echo build's scaling stat (`scalingStatFromSkillTree`).                             |
| `skillBonusPriority`       | UI hint + echo substat router target. A `SubstatSkillType` — one of the four DMG-bonus substat skill types: Basic Attack, Heavy Attack, Resonance Skill, Resonance Liberation. The default substat block routes its Skill DMG Bonus rolls here. |
