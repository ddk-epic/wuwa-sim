...
**Key fields**: multiple entries in `stages[]`, each its own stage with a distinct `newName` (`"Stage 1"`, `"Stage 2"`, …). They share the parent skill's `name`, so their stageIds differ only in the `<stage>` segment. **`requiresStageId`** enforces the order: gate every stage after the first on its predecessor's full lineage stageId (exact match), and leave stage 1 ungated. The chain only advances when the prior stage was the one that just played.

**Gotchas**: `newName` is what disambiguates the stages in both the UI label and the stageId. Two stages with the same `category` **and** the same `newName` would collide on stageId — keep `newName` unique within a skill. `requiresStageId` matches the predecessor's id **exactly** (not by prefix).
...
```ts
{
  ...
  stages: [
    ...
    },
    {
      ...
      requiresStageId: "char.sanhua.basic-attack.frigid-light.stage-1::basic-attack",
      actionTime: 31,
      value: "73.76%",
      damage: [/* … */],
    },
...
| `newName`         | Display label **and** the `<stageName>` stageId segment. `""`/absent → `_`. Generated by `deriveNewName` (`"Skill DMG"` → `""`, trailing `" DMG"`/`" Damage"` stripped); override only when the derived label collides within a skill+category. |
...
| `requiresStageId` | Combo-string gating — this stage only follows the given predecessor stageId (exact match).                                                                                                                                                      |
| `comboAllows`     | Movement (`Dodge`/`Jump`) interruptions permitted mid-chain without breaking it. Only honored when `requiresStageId` is set.                                                                                                                    |

**Rare fields:**

- `comboAllows` — declares which `MovementKind`s (`Dodge`/`Jump`) may interrupt a `requiresStageId` chain without breaking it; any other intervening stage breaks the chain. Only honored alongside `requiresStageId` (the chain itself is Cookbook recipe 3). No authored example yet.
...
A buff trigger references a stage by an id you **never write on the stage** — it's composed from stage and skill fields by `makeCharStageId` (`src/lib/stage.ts`). Knowing the rule lets you predict the id when wiring a buff.

```
char.sanhua.basic-attack.frigid-light.stage-5::basic-attack.1
 │     │         │             │           │         │        │
 │     │         │             │           │         │        └─ hit index, 1-based — appended per emitted
 │     │         │             │           │         │           hit (DamageEntry order), NOT in makeCharStageId
 │     │         │             │           │         └─ skillType: damage[0].type ?? category
 │     │         │             │           │            (Tune Break → Basic Attack)
 │     │         │             │           └─ stage.newName  ("" / absent → "_")
 │     │         │             └─ skill.name
 │     │         └─ stage.category  (the SkillCategory / trigger axis)
 │     └─ character.name
 └─ literal "char"
```

All segments are kebab-cased (`toKebab`): camelCase split, lowercased, non-alphanumerics → `-`. An empty/undefined segment becomes `_`.

Key points an author trips on:

- The `<skill>` segment is the **parent skill's `name`**; the `<stage>` segment is the **stage's `newName`** (not its `name`). A single-cast skill with `newName: ""` produces a `_` segment: `char.sanhua.resonance-skill.eternal-frost._::resonance-skill`.
